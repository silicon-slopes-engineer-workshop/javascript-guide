...
## Install `Node`
`Node` is the the runtime environment for executing JavaScript code from the command-line. 
By using `Node`, you can create any type of application using JavaScript including server-side / backend applications.
...
`tsc` is the TypeScript compiler. We use tsc to compile TypeScript code into JavaScript code. 
We can install the TypeScript compile using the Node Package Manager (`npm`)
...
> Not me, not the Chris that Luv2Code send thanks to LOL